...
if [ "$INSTALL_NODERED" = true ]; then
    ...
    # Create package.json so Node-RED loads NCD nodes on startup
    cat > ~/.node-red/package.json << 'PKGEOF'
{
    "name": "node-red-project",
    "description": "NCD.io Sensor Setup",
    "version": "0.0.1",
    "private": true,
    "dependencies": {
        "node-red": "3.1.15",
...
PKGEOF
    ...
    # Make node-red command available system-wide
    sudo ln -sf "$HOME/.node-red/node_modules/.bin/node-red" /usr/bin/node-red
    sudo usermod -a -G dialout "$USER"
    echo "  ✓ Node-RED v3.1.15 installed"
    echo "  ✓ NCD enterprise sensors package installed"
    echo "  ✓ Added $USER to dialout group"
    echo ""
    echo "  To start Node-RED: node-red &"
    echo "  Then open: http://localhost:1880"
    echo "  Import flow and type port: /dev/ttyUSB0"
