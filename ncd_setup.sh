#!/bin/bash
# =============================================================================
# NCD.io Wireless Sensor — Raspberry Pi OS Setup Script
...
echo "============================================="
echo " NCD.io Raspberry Pi Setup Script"
...
# =============================================================================
# STEP 2 — NVM + Node.js v18 (system-wide)
# =============================================================================
echo ""
echo "[2/6] Installing NVM and Node.js v18..."
...
# =============================================================================
# STEP 4 — NCD package + serialport fix + dialout
# =============================================================================
echo ""
echo "[4/6] Installing NCD enterprise sensors..."
cd "$HOME/.node-red"
...
# =============================================================================
# STEP 5 — Screenshot shortcuts (Wayland / labwc)
# =============================================================================
echo ""
echo "[5/6] Configuring screenshot shortcuts..."
...
# =============================================================================
# STEP 6 — Auto-detect serial port + patch and write flow JSON
# =============================================================================
echo ""
echo "[6/6] Detecting NCD USB modem and writing patched flow..."

# --- Auto-detect serial port ---
SERIAL_PORT=""

# Method 1: FTDI device via dmesg (NCD modem uses FTDI chip)
FTDI_PORT=$(dmesg 2>/dev/null | grep -i "FTDI\|ftdi_sio" | grep -oP 'ttyUSB\d+' | tail -1 || true)
if [ -n "$FTDI_PORT" ]; then
    SERIAL_PORT="/dev/$FTDI_PORT"
    echo "  ✓ FTDI device found via dmesg: $SERIAL_PORT"
fi

# Method 2: /dev/serial/by-id symlink for FTDI
if [ -z "$SERIAL_PORT" ]; then
    ...
    if [ -n "$FTDI_LINK" ]; then
        ...
        echo "  ✓ FTDI device found via /dev/serial/by-id: $SERIAL_PORT"
    fi
fi

# Method 3: first available ttyUSB
if [ -z "$SERIAL_PORT" ]; then
    FIRST_USB=$(ls /dev/ttyUSB* 2>/dev/null | head -1 || true)
    if [ -n "$FIRST_USB" ]; then
        SERIAL_PORT="$FIRST_USB"
        echo "  ⚠ No FTDI ID found, using first ttyUSB: $SERIAL_PORT"
    fi
fi
...
if [ -z "$SERIAL_PORT" ]; then
    ...
    echo "  ⚠ No USB serial device detected. Defaulting to $SERIAL_PORT"
    echo "    Plug in the NCD modem and reboot if the gateway shows not connected."
fi
...
for node in flow:
    if node.get('type') == 'ncd-gateway-config':
        ...
        patched += 1
        print(f"  Patched [{node.get('name')}] port -> {serial_port}")
    if node.get('type') == 'file':
        node['filename'] = csv_path
        patched += 1
        print(f"  Patched [{node.get('name')}] filename -> {csv_path}")
    if node.get('type') == 'function' and node.get('func',''):
        if 'PLACEHOLDER_CSV' in node['func']:
            node['func'] = node['func'].replace('PLACEHOLDER_CSV', csv_path)
            patched += 1
            print(f"  Patched [{node.get('name')}] msg.filename -> {csv_path}")

with open(output_path, 'w') as f:
    json.dump(flow, f, indent=4)

print(f"  {patched} items patched. Flow saved to: {output_path}")
PYEOF
